...
const addNewConsole = async (req, res) => {
    try {
        const {name, developer, releaseDate, 
            display, storageCapacities, numberOfPlayers, available, description}= req.body

            const newConsole = new ConsolesModel({name, developer, releaseDate, 
                display, storageCapacities, numberOfPlayers, available, description})
                const savedConsole = await newConsole.save()
                res.status(201).json({message:"Seu console foi cadastrado", savedConsole })
    } catch (error) {
        console.error(error)
        res.status(500).json({message: error.message})
    }
